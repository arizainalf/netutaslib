...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            datatableCall('loan-table', '{{ route('loan.index') }}', [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex'
                },
                {
            ...
            select2ToJson("#member_id", "{{ route('member.index') }}", "#createModal");
            select2ToJson("#book_id", "{{ route('book.index') }}", "#createModal");

            $("#saveData").submit(function(e) {
                setButtonLoadingState("#saveData .btn.btn-success", true);
                e.preventDefault();
                const kode = $("#saveData #id").val();
                let url = "{{ route('loan.store') }}";
                ...
                const successCallback = function(response) {
                    ...
                    handleSuccess(response, "loan-table", "createModal");
                };

                const errorCallback = function(error) {
                    setButtonLoadingState("#saveData .btn.btn-success", false);
                    handleValidationErrors(error, "saveData", ['member_id', 'book_id',
